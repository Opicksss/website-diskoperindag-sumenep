...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="d-flex justify-content-between align-items-center">
            <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Koperasi /</span> Fasilitas</h4>
        </div>
        <!-- Striped Rows -->
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="card-title mb-0">Data Fasilitas Pendirian Koperasi</h5>
                            ...
                            <div class="d-flex justify-content-end">
                                <button type="button" class="btn btn-primary " data-bs-toggle="modal"
                                    ...
                                    <span><i class="bx bx-plus"></i> <span
                                            class="d-none d-sm-inline-block">Fasilitas</span></span>
                                </button>
                            </div>
                        </form>
                    </div>
                    ...
                    <tbody class="table-border-bottom-0">
                        @foreach ($fasilitass as $fasilitas)
                            <tr>
                                ...
                                <td><strong>{{ ucwords($fasilitas->title) }}</strong></td>
                                <td>{!! Str::limit($fasilitas->description, 100) !!}</td>
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu">
                                            <a class="dropdown-item"
                                                href="{{ route('adminfasilitas.show', $fasilitas->id) }}"><i
                                                    class="bx bx-show-alt me-1"></i> Show</a>
                                            <form id="delete-form-{{ $fasilitas->id }}"
                                                action="{{ route('adminfasilitas.destroy', $fasilitas->id) }}"
                                                method="post">
                                                @csrf
                                                @method('delete')
                                                <button type="button" class="dropdown-item text-danger delete-btn"
                                                    data-fasilitas-id="{{ $fasilitas->id }}"
                                                    data-fasilitas-title="{{ ucwords($fasilitas->title) }}">
            ...
            <div class="d-flex justify-content-end">
                {{ $fasilitass->links('layouts.pagination') }}
            </div>
        </div>
        <!--/ Striped Rows -->
    </div>
        ...
        <div class="modal-dialog modal-xl" role="document">
            <form action="{{ route('adminfasilitas.store') }}" method="post" enctype="multipart/form-data"
                ...
                <div class="modal-header">
                    <h5 class="modal-title" id="backDropModalTitle">Tambah Fasilitas</h5>
