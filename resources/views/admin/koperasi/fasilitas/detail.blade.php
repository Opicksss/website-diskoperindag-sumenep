...
@section('title')
    Admin Fasilitas {{ ucwords($fasilitas->title) }}
@endsection
...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="d-flex justify-content-between align-items-center">
            <h4 class="fw-bold py-3 mb-4">
                ...
                <span><a href="/adminfasilitas" class="text-muted"> Fasilitas /</a></span>
                {{ ucwords($fasilitas->title) }}
            </h4>
        </div>
        <div class="row">
            <!-- Cover Image -->
            ...
            <div class="col-md-8">
                <div class="card shadow mb-4">
                    <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between bg-body">
                        <h6 class="m-0 fw-bold">Detail Fasilitas {{ ucwords($fasilitas->title) }}</h6>
                        <a class="btn-close " href="/adminfasilitas"></a>
                    ...
                    <div class="card-body py-3">
                        <table>
                            <tr>
                                <td>Nama </td>
                                <td> : {{ ucwords($fasilitas->title) }}</td>
                            </tr>
                            <tr>
                                <td>Tanggal </td>
                                <td> : {{ date('d-m-Y', strtotime($fasilitas->tanggal)) }}</td>
                            </tr>
                        </table>
                    </div>
                    <!-- proses -->
                    ...
                    <div class="card-body py-3 d-flex align-items-start gap-2 ">
                        <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                            data-bs-target="#ModalEdit{{ $fasilitas->id }}">
                            <i class="bx bx-edit-alt "></i></button>
                        <form id="delete-form-{{ $fasilitas->id }}"
                            action="{{ route('adminfasilitas.destroy', $fasilitas->id) }}" method="post">
                            @csrf
                            @method('delete')
                            <button type="button" class="btn btn-danger delete-btn"
                                data-fasilitas-id="{{ $fasilitas->id }}"
                                data-fasilitas-title="{{ $fasilitas->title }}">
    ...
    <!-- Modal Edit -->
    <div class="modal fade" id="ModalEdit{{ $fasilitas->id }}" data-bs-backdrop="static" tabindex="-1">
        <div class="modal-dialog modal-xl" role="document"">
            <div class="modal-content">
                <form action="{{ route('adminfasilitas.update', $fasilitas->id) }}" method="post"
                    enctype="multipart/form-data">
                    @csrf
                    @method('put')
                    <div class="modal-header">
                        <h5 class="modal-title" id="backDropModalTitle">Edit Fasilitas</h5>
                        ...
                        <div class="row g-2">
                            <div class="col mb-3">
                                ...
                                <input type="text" class="form-control" id="title" name="title"
                                    value="{{ $fasilitas->title }}" required>
                            </div>
                            <div class="col mb-3">
                                <label for="tanggal" class="form-label">Tanggal</label>
                                <input type="date" class="form-control" id="tanggal" name="tanggal"
                                    value="{{ $fasilitas->tanggal }}" required>
                            </div>
                        </div>
                        <div class="mb-3">
                            <label class="form-label" for="description">Deskripsi</label>
                            <textarea class="form-control" id="description" rows="7" name="description" required>{{ $fasilitas->description }}</textarea>
                        </div>
                        @if ($fasilitas->fotos->isEmpty())
                        @else
                            <div class="mb-3">
    ...
    <script>
            ...
            document.querySelectorAll('.delete-btn').forEach(function(button) {
                button.addEventListener('click', function() {
                    const bookId = this.getAttribute('data-fasilitas-id');
                    const bookTitle = this.getAttribute('data-fasilitas-title');
                    Swal.fire({
                        title: 'Apakah kamu yakin?',
                        text: `Data Fasilitas Dengan Judul "${bookTitle}" akan dihapus`,
                        icon: 'warning',
