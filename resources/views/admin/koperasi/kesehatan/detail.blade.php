...
@section('title')
    Admin Kesehatan {{ ucwords($kesehatan->title) }}
@endsection
...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="d-flex justify-content-between align-items-center">
            <h4 class="fw-bold py-3 mb-4">
                ...
                <span><a href="/adminkesehatan" class="text-muted"> Kesehatan /</a></span>
                {{ ucwords($kesehatan->title) }}
            </h4>
        </div>
        <div class="row">
            <!-- Cover Image -->
            ...
            <div class="col-md-8">
                <div class="card shadow mb-4">
                    <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between bg-body">
                        <h6 class="m-0 fw-bold">Detail Kesehatan {{ ucwords($kesehatan->title) }}</h6>
                        <a class="btn-close " href="/adminkesehatan"></a>
                    </div>
                    <!-- Card Body -->
                    <div class="card-body py-3">
                        <table>
                            <tr>
                                <td>Nama </td>
                                <td> : {{ ucwords($kesehatan->title) }}</td>
                            </tr>
                            <tr>
                                <td>Tanggal </td>
                                <td> : {{ date('d-m-Y', strtotime($kesehatan->tanggal)) }}</td>
                            </tr>
                        </table>
                    </div>
                    <!-- proses -->
                    ...
                    <div class="card-body py-3 d-flex align-items-start gap-2 ">
                        <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                            data-bs-target="#ModalEdit{{ $kesehatan->id }}">
                            <i class="bx bx-edit-alt "></i></button>
                        <form id="delete-form-{{ $kesehatan->id }}"
                            action="{{ route('adminkesehatan.destroy', $kesehatan->id) }}" method="post">
                            @csrf
                            @method('delete')
                            <button type="button" class="btn btn-danger delete-btn"
                                data-kesehatan-id="{{ $kesehatan->id }}"
                                data-kesehatan-title="{{ $kesehatan->title }}">
    ...
    <!-- Modal Edit -->
    <div class="modal fade" id="ModalEdit{{ $kesehatan->id }}" data-bs-backdrop="static" tabindex="-1">
        <div class="modal-dialog modal-xl" role="document"">
            <div class="modal-content">
                <form action="{{ route('adminkesehatan.update', $kesehatan->id) }}" method="post"
                    enctype="multipart/form-data">
                    @csrf
                    @method('put')
                    <div class="modal-header">
                        <h5 class="modal-title" id="backDropModalTitle">Edit Kesehatan</h5>
                        ...
                        <div class="row g-2">
                            <div class="col mb-3">
                                ...
                                <input type="text" class="form-control" id="title" name="title"
                                    value="{{ $kesehatan->title }}" required>
                            </div>
                            <div class="col mb-3">
                                <label for="tanggal" class="form-label">Tanggal</label>
                                <input type="date" class="form-control" id="tanggal" name="tanggal"
                                    value="{{ $kesehatan->tanggal }}" required>
                            </div>
                        </div>
                        <div class="mb-3">
                            <label class="form-label" for="description">Deskripsi</label>
                            <textarea class="form-control" id="description" rows="7" name="description" required>{{ $kesehatan->description }}</textarea>
                        </div>
                        @if ($kesehatan->fotos->isEmpty())
                        @else
                            <div class="mb-3">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            document.querySelectorAll('.delete-btn').forEach(function(button) {
                button.addEventListener('click', function() {
                    const bookId = this.getAttribute('data-kesehatan-id');
                    const bookTitle = this.getAttribute('data-kesehatan-title');
                    Swal.fire({
                        title: 'Apakah kamu yakin?',
                        text: `Data Kesehatan Dengan Judul "${bookTitle}" akan dihapus`,
                        icon: 'warning',
