...
@section('title')
    Admin Pengawasan {{ ucwords($pengawasan->title) }}
@endsection
...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="d-flex justify-content-between align-items-center">
            <h4 class="fw-bold py-3 mb-4">
                ...
                <span><a href="/adminpengawasan" class="text-muted"> Pengawasan /</a></span>
                {{ ucwords($pengawasan->title) }}
            </h4>
        </div>
        <div class="row">
            <!-- Cover Image -->
            ...
            <div class="col-md-8">
                <div class="card shadow mb-4">
                    <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between bg-body">
                        <h6 class="m-0 fw-bold">Detail Pengawasan {{ ucwords($pengawasan->title) }}</h6>
                        <a class="btn-close " href="/adminpengawasan"></a>
                    ...
                    <div class="card-body py-3">
                        <table>
                            <tr>
                                <td>Nama </td>
                                <td> : {{ ucwords($pengawasan->title) }}</td>
                            </tr>
                            <tr>
                                <td>Tanggal </td>
                    ...
                    <div class="card-body py-3 d-flex align-items-start gap-2 ">
                        <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                            data-bs-target="#ModalEdit{{ $pengawasan->id }}">
                            <i class="bx bx-edit-alt "></i></button>
                        <form id="delete-form-{{ $pengawasan->id }}"
                            action="{{ route('adminpengawasan.destroy', $pengawasan->id) }}" method="post">
                            @csrf
                            @method('delete')
                            <button type="button" class="btn btn-danger delete-btn"
                                data-pengawasan-id="{{ $pengawasan->id }}"
                                data-pengawasan-title="{{ $pengawasan->title }}">
    ...
    <!-- Modal Edit -->
    <div class="modal fade" id="ModalEdit{{ $pengawasan->id }}" data-bs-backdrop="static" tabindex="-1">
        <div class="modal-dialog modal-xl" role="document"">
            <div class="modal-content">
                <form action="{{ route('adminpengawasan.update', $pengawasan->id) }}" method="post"
                    enctype="multipart/form-data">
                    @csrf
                    @method('put')
                    <div class="modal-header">
                        <h5 class="modal-title" id="backDropModalTitle">Edit Pengawasan</h5>
                        ...
                        <div class="row g-2">
                            <div class="col mb-3">
                                ...
                                <input type="text" class="form-control" id="title" name="title"
                                    value="{{ $pengawasan->title }}" required>
                            </div>
                            <div class="col mb-3">
                                <label for="tanggal" class="form-label">Tanggal</label>
                                <input type="date" class="form-control" id="tanggal" name="tanggal"
                                    value="{{ $pengawasan->tanggal }}" required>
                            </div>
                        </div>
                        <div class="mb-3">
                            <label class="form-label" for="description">Deskripsi</label>
                            <textarea class="form-control" id="description" rows="7" name="description" required>{{ $pengawasan->description }}</textarea>
                        </div>
                        @if ($pengawasan->fotos->isEmpty())
                        @else
                            <div class="mb-3">
