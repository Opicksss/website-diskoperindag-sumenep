...
@section('title')
    Admin Penghargaan {{ ucwords($penghargaan->title) }}
@endsection
...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="d-flex justify-content-between align-items-center">
            <h4 class="fw-bold py-3 mb-4">
                ...
                <span><a href="/adminpenghargaan" class="text-muted"> Penghargaan /</a></span>
                {{ ucwords($penghargaan->title) }}
            </h4>
        </div>
        <div class="row">
            <!-- Cover Image -->
            ...
            <div class="col-md-8">
                <div class="card shadow mb-4">
                    <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between bg-body">
                        <h6 class="m-0 fw-bold">Detail Penghargaan {{ ucwords($penghargaan->title) }}</h6>
                        <a class="btn-close " href="/adminpenghargaan"></a>
                    ...
                    <div class="card-body py-3">
                        <table>
                            <tr>
                                <td>Nama </td>
                                <td> : {{ ucwords($penghargaan->title) }}</td>
                            </tr>
                            <tr>
                                <td>Tanggal </td>
                                <td> : {{ date('d-m-Y', strtotime($penghargaan->tanggal)) }}</td>
                            </tr>
                        </table>
                    </div>
                    <!-- proses -->
                    ...
                    <div class="card-body py-3 d-flex align-items-start gap-2 ">
                        <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                            data-bs-target="#ModalEdit{{ $penghargaan->id }}">
                            <i class="bx bx-edit-alt "></i></button>
                        <form id="delete-form-{{ $penghargaan->id }}"
                            action="{{ route('adminpenghargaan.destroy', $penghargaan->id) }}" method="post">
                            @csrf
                            @method('delete')
                            <button type="button" class="btn btn-danger delete-btn"
                                data-penghargaan-id="{{ $penghargaan->id }}"
                                data-penghargaan-title="{{ $penghargaan->title }}">
    ...
    <!-- Modal Edit -->
    <div class="modal fade" id="ModalEdit{{ $penghargaan->id }}" data-bs-backdrop="static" tabindex="-1">
        <div class="modal-dialog modal-xl" role="document"">
            <div class="modal-content">
                <form action="{{ route('adminpenghargaan.update', $penghargaan->id) }}" method="post"
                    enctype="multipart/form-data">
                    @csrf
                    @method('put')
                    <div class="modal-header">
                        <h5 class="modal-title" id="backDropModalTitle">Edit Penghargaan</h5>
                    ...
                    <div class="modal-body">
                        <div class="row g-2">
                            <div class="col mb-3">
                                ...
                                <input type="text" class="form-control" id="title" name="title"
                                    value="{{ $penghargaan->title }}" required>
                            </div>
                            <div class="col mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="description" rows="7" name="description" required>{{ $penghargaan->description }}</textarea>
                        </div>
                        @if ($penghargaan->fotos->isEmpty())
                        @else
                            <div class="mb-3">
    ...
    <script>
            ...
            document.querySelectorAll('.delete-btn').forEach(function(button) {
                button.addEventListener('click', function() {
                    const bookId = this.getAttribute('data-penghargaan-id');
                    const bookTitle = this.getAttribute('data-penghargaan-title');
                    Swal.fire({
                        title: 'Apakah kamu yakin?',
                        text: `Data Penghargaan Dengan Judul "${bookTitle}" akan dihapus`,
                        icon: 'warning',
