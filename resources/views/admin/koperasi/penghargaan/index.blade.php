...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="d-flex justify-content-between align-items-center">
            <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Koperasi /</span> Penghargaan</h4>
        </div>
        <!-- Striped Rows -->
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="card-title mb-0">Data Penghargaan Yang Telah Di Raih</h5>
                            ...
                            <div class="d-flex justify-content-end">
                                <button type="button" class="btn btn-primary " data-bs-toggle="modal"
                                    ...
                                    <span><i class="bx bx-plus"></i> <span
                                            class="d-none d-sm-inline-block">Penghargaan</span></span>
                                </button>
                            </div>
                        </form>
                    </div>
                    ...
                    <tbody class="table-border-bottom-0">
                        @foreach ($penghargaans as $penghargaan)
                            <tr>
                                ...
                                <td><strong>{{ ucwords($penghargaan->title) }}</strong></td>
                                <td>{!! Str::limit($penghargaan->description, 100) !!}</td>
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu">
                                            <a class="dropdown-item"
                                                href="{{ route('adminpenghargaan.show', $penghargaan->id) }}"><i
                                                    class="bx bx-show-alt me-1"></i> Show</a>
                                            <form id="delete-form-{{ $penghargaan->id }}"
                                                action="{{ route('adminpenghargaan.destroy', $penghargaan->id) }}"
                                                method="post">
                                                @csrf
                                                @method('delete')
                                                <button type="button" class="dropdown-item text-danger delete-btn"
                                                    data-penghargaan-id="{{ $penghargaan->id }}"
                                                    data-penghargaan-title="{{ ucwords($penghargaan->title) }}">
            ...
            <div class="d-flex justify-content-end">
                {{ $penghargaans->links('layouts.pagination') }}
            </div>
        </div>
        <!--/ Striped Rows -->
    </div>
    ...
    <div class="modal fade" id="ModalCreate" data-bs-backdrop="static" tabindex="-1">
        <div class="modal-dialog modal-xl" role="document">
            <form action="{{ route('adminpenghargaan.store') }}" method="post" enctype="multipart/form-data"
                class="modal-content">
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title" id="backDropModalTitle">Tambah Penghargaan</h5>
