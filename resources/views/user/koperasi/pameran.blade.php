...
@section('content')
    ...
    <div class="container-fluid bg-breadcrumb">
        <div class="container text-center py-5" style="max-width: 900px;">
            <h4 class="text-white display-4 mb-4 wow fadeInDown" data-wow-delay="0.1s">PAMERAN YANG PERNAH DIIKUTI</h4>
            <ol class="breadcrumb d-flex justify-content-center mb-0 wow fadeInDown" data-wow-delay="0.3s">
                <li class="breadcrumb-item"><a href="/">Home</a></li>
                <li class="breadcrumb-item text-white">Dinas Koperasi</li>
                <li class="breadcrumb-item active text-white-50">Pameran</li>
            </ol>
        </div>
    </div>
    <!-- Header End -->
                        ...
                        <div class="team-item">
                            <div class="team-img">
                                @if ($pameran->fotos->isNotEmpty())
                                    <img class="card-img-top" src="/storage/{{ $pameran->fotos->first()->foto }}"
                            ...
                            <div class="team-title blog-content p-4">
                                <h4 class="d-inline-block mb-1">{{ ucwords($pameran->title) }}</h4>
                                <div class="content-text">
                                    {!! ($pameran->description) !!}
                                </div>
                                <div class="date-underline text-right">
                                    <p>{{ date('d-m-Y', strtotime($pameran->tanggal)) }}</p>
                                </div>
                                <a class="btn btn-primary rounded-pill py-2 px-4"
                                    href="{{ route('show.pameran', $pameran->id) }}">Read More</a>
                            </div>
                        </div>
                    </div>
                    @php
